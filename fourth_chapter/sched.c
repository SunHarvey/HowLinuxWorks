...
int main(int argc,char *argv[1])
{
	...
	if (nproc < 1) {
		fprintf(stderr, "<nproc>(%d) shoud be >= 1\n",nproc);
		exit(EXIT_FAILURE);
	}
	if (total < 1) {
		fprintf(stderr, "<total>(%d) shoud be >= 1\n",total);
		exit(EXIT_FAILURE);
	}
	if (resol < 1) {
		fprintf(stderr, "<resol>(%d) shoud be >= 1\n",resol);
		exit(EXIT_FAILURE);
	}
	if (total % resol) {
		fprintf(stderr, "<resol>(%d) shoud be multiple of <resolution>(%d)\n", total, resol);
		exit(EXIT_FAILURE);
	}
	...
	puts("estimat wordload which takes just one milisecond");
